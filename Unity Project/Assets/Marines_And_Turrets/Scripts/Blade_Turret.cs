...
public class Blade_Turret : MonoBehaviour {
	Animator anim;
	...
	//--OnGUI Interface--
	void OnGUI() {
		/*if (GUI.Button (new Rect (200,20, 110, 20), "Autodestruct in: ")) {}
		if (GUI.Button (new Rect (310,20, 50, 20), "" + Duration)) {}*/
	...
	void OnCollisionEnter2D(Collision2D coll) {
		if(is_activated == false){
			...
			Destroy (GameObject.Find("Grenade_Target2"));
		}
	}
