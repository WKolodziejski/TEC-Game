...
//The collisions are deactivated (collisions disabled and kinematic enabled) until the turret is not in "activated animation"
//The collisions are deactivated when the turret is in "deactivated animation"
public class Shield_Turret : MonoBehaviour {
	Animator anim;
	...
	void OnCollisionEnter2D(Collision2D coll) {
		if(is_activated == false){
			...
			Destroy (GameObject.Find("Grenade_Target3"));
		}
	}
	...
	void Disable_Collision(){
		if (this.rigidbody2D.isKinematic == false) {
			this.rigidbody2D.isKinematic = !this.rigidbody2D.isKinematic;
			this.gameObject.collider2D.enabled = !this.gameObject.collider2D.enabled;
		}
	}
	
	void Enable_Collision(){
		if (this.rigidbody2D.isKinematic == true) {
			this.rigidbody2D.isKinematic = !this.rigidbody2D.isKinematic;
			this.gameObject.collider2D.enabled = !this.gameObject.collider2D.enabled;
		}
	}
